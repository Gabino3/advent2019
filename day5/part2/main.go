...
func processInput(input []int) (int, error) {
	...
	for i := 0; i < len(input); {
		...
		case 4:
			...
			instructionStep = 2
		case 5:
			if getParamValue(input, i+1, getDigit(input[i], firstParamPosition)) != 0 {
				i = getParamValue(input, i+2, getDigit(input[i], secondParamPosition))
				continue
			}
			instructionStep = 3
		case 6:
			...
			instructionStep = 3
		case 7:
			if getParamValue(input, i+1, getDigit(input[i], firstParamPosition)) < getParamValue(input, i+2, getDigit(input[i], secondParamPosition)) {
				input[input[i+3]] = 1
			} else {
				input[input[i+3]] = 0
			}
