	...
	for _, movement := range arrPath {
		...
		case 'R':
			for i := 0; i < count; i++ {
				...
				positions[currentPosition.String()] = currentPosition
			}
		case 'U':
			for i := 0; i < count; i++ {
				currentPosition.Y += 1
...
func findIntersections(positionsA, positionsB map[string]Position) []Position {
	intersecs := make([]Position, 0)
	for posAKey, posA := range positionsA {
		if posB, ok := positionsB[posAKey]; ok == true {
			posA.stepCount += posB.stepCount
			intersecs = append(intersecs, posA)
		}
	}
	return intersecs
}

func getShortestStepCount(positions []Position) int {
	minStepCount := 99999999999999999

	for _, pos := range positions {

		if pos.stepCount < minStepCount {
			minStepCount = pos.stepCount
		}
