...
func main() {
	...
	fmt.Printf("%d", getShortestManhattenDistance(intersecs))

}

func strPathToPos(strPath string) map[Position]bool {
	...
	for _, movement := range arrPath {
		if movement == "" {
			return make(map[Position]bool, 0)
		}
...
func findIntersections(positionsA, positionsB map[Position]bool) []Position {
	intersecs := make([]Position, 0)
	for posA := range positionsA {
		if positionsB[posA] == true {
...
func getShortestManhattenDistance(positions []Position) int {
	minManhattenDistance := float64(999999999999999999.0)

	for _, pos := range positions {
		mDistance := math.Abs(0.0-float64(pos.X)) + math.Abs(0.0-float64(pos.Y))
		if mDistance < minManhattenDistance {
			minManhattenDistance = mDistance
		}

	}

	return int(minManhattenDistance)
}
